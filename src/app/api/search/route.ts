...
import { BlogsApi, ArticlesApi, CoursesApi, AuthorsApi, BooksApi, EventsApi, IftahApi, AwlyaaApi, TasawwufApi } from '@/lib/api';
...
function generateUrl(type: SearchResult['type'], data: any): string {
  switch (type) {
    case 'article':
...
function extractTitle(data: any): string {
  return data.title || data.name || data.question || 
...
function extractDescription(data: any): string | undefined {
  return data.description || data.excerpt || data.answer || data.bio || undefined;
}
...
function matchesQuery(result: SearchResult, query: string): boolean {
  ...
  const stopWords = new Set([
    'د', 'دی', 'څو', 'ته', 'وايې', 'او', 'یا', 'چی', 'کې', 'په', 'نه', 'دا', 'دغه',
    'هغه', 'یو', 'دوه', 'درې', 'څلور', 'پنځه', 'شپږ', 'اووم', 'اتم', 'نهم', 'لسه',
    'کول', 'کړل', 'کړې', 'کړي', 'کېږي', 'کېدل', 'کېږې', 'کېدي', 'شو', 'شوه', 'شوي',
    'کېږي', 'کېږې', 'کېږي', 'وي', 'وو', 'وې', 'وي', '؟', '!', '.', ',', ';', ':'
  ]);
  ...
  if (searchTerms.length > 0) {
    const matchedWords = searchTerms.filter(term => searchableText.includes(term));
...
function transformSearchResult(item: any, modelType: string): SearchResult | null {
  try {
    ...
    return {
      ...
      slug: item.slug,
      url: generateUrl(type, item),
      image: item.image || item.featuredImage || item.photo,
      date: item.created_at || item.publishedAt || item.date || item.written_year,
      author: item.author?.name || 
              (item.author ? `${item.author.first_name || ''} ${item.author.last_name || ''}`.trim() : undefined) ||
              (item.recorded_by ? `${item.recorded_by.first_name || ''} ${item.recorded_by.last_name || ''}`.trim() : undefined),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const searchUrl = `${endpoints.searchGlobal}?q=${encodeURIComponent(query.trim())}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorJson.message) {
            ...
            if (errorJson.message.includes('Column not found') || errorJson.message.includes('Unknown column')) {
              errorMessage = 'Database schema issue in search API. Please check the Laravel SearchController.';
            }
          }
        } catch {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        console.error('Search API returned non-JSON response:', text.substring(0, 500));
        throw new Error('Search API returned non-JSON response');
      }

      laravelResponse = await response.json();
    } catch (fetchError) {
      console.warn('Global search failed, falling back to individual API searches:', fetchError);
      ...
      const stopWords = new Set([
        'د', 'دی', 'څو', 'ته', 'وايې', 'او', 'یا', 'چی', 'کې', 'په', 'نه', 'دا', 'دغه',
        'هغه', 'یو', 'دوه', 'درې', 'څلور', 'پنځه', 'شپږ', 'اووم', 'اتم', 'نهم', 'لسه',
        'کول', 'کړل', 'کړې', 'کړي', 'کېږي', 'کېدل', 'کېدې', 'کېدي', 'شو', 'شوه', 'شوي',
        'کېږي', 'کېږې', 'کېږي', 'وي', 'وو', 'وې', 'وي', '؟', '!', '.', ',', ';', ':'
      ]);
      ...
      const matchesSearch = (item: any, fields: string[]): boolean => {
        const searchableText = fields
          .map(field => {
            ...
            return value ? String(value).toLowerCase().replace(/[؟!.,;:]+/g, '') : '';
          })
          .join(' ');
      ...
      try {
        ...
        const [blogsRes, articlesRes, coursesRes, authorsRes, booksRes, eventsRes, iftahRes, awlyaaRes, tasawwufRes] = await Promise.allSettled([
        ...
        if (blogsRes.status === 'fulfilled' && blogsRes.value.success) {
          ...
          blogs.forEach((blog: any) => {
            if (matchesSearch(blog, ['title', 'description', 'excerpt', 'author.name'])) {
              results.push({
                type: 'blog',
                id: blog.id,
                title: blog.title || '',
        ...
        if (articlesRes.status === 'fulfilled' && articlesRes.value.success) {
          ...
          articles.forEach((article: any) => {
            if (matchesSearch(article, ['title', 'description', 'excerpt', 'author.name'])) {
              results.push({
                type: 'article',
                id: article.id,
                title: article.title || '',
        ...
        if (booksRes.status === 'fulfilled' && booksRes.value.success) {
          ...
          books.forEach((book: any) => {
            if (matchesSearch(book, ['title', 'description', 'author.first_name', 'author.last_name'])) {
              results.push({
                type: 'book',
                id: book.id,
                title: book.title || '',
        ...
        if (awlyaaRes.status === 'fulfilled' && awlyaaRes.value.success) {
          const awlyaas = Array.isArray(awlyaaRes.value.data) ? awlyaaRes.value.data : [];
          awlyaas.forEach((awlyaa: any) => {
            if (matchesSearch(awlyaa, ['name', 'first_name', 'last_name', 'description', 'bio'])) {
              const name = awlyaa.name || `${awlyaa.first_name || ''} ${awlyaa.last_name || ''}`.trim();
              results.push({
                type: 'awlyaa',
                id: awlyaa.id,
                title: name || 'Untitled',
        ...
        if (tasawwufRes.status === 'fulfilled' && tasawwufRes.value.success) {
          const tasawwufs = Array.isArray(tasawwufRes.value.data) ? tasawwufRes.value.data : [];
        ...
        if (fetchError instanceof Error && fetchError.message.includes('fetch failed')) {
          throw new Error('Unable to connect to search API. Please check if the API server is running.');
        }
        throw fetchError;
      }
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('Laravel search response keys:', Object.keys(laravelResponse));
      console.log('Laravel search response sample:', JSON.stringify(laravelResponse).substring(0, 1000));
    }
    ...
    if (laravelResponse.data && Array.isArray(laravelResponse.data)) {
      ...
      laravelResponse.data.forEach((item: any) => {
        const modelType = item.model_type || item.type || item.model || 'article';
        const transformed = transformSearchResult(item, modelType);
        if (transformed) {
          results.push(transformed);
        }
      });
    } else if (laravelResponse.results && Array.isArray(laravelResponse.results)) {
    ...
    } else if (laravelResponse.articles || laravelResponse.blogs || laravelResponse.courses || 
               laravelResponse.awlyaa || laravelResponse.tasawwuf || laravelResponse.authors || 
               laravelResponse.books || laravelResponse.events || laravelResponse['darul-ifta']) {
      ...
      console.warn('Unknown Laravel response structure. Keys:', Object.keys(laravelResponse));
    ...
    const filteredResults = results.filter(result => matchesQuery(result, query));
    ...
    console.error('Error details:', {
      ...
      query: query,
      searchUrl: `${endpoints.searchGlobal}?q=${encodeURIComponent(query.trim())}`,
    });
