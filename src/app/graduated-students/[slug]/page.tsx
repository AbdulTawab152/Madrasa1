...
export default function GraduationDetailPage({ params }: { params: { slug: string } }) {
  ...
  const currentImageUrl = useMemo(() => {
    ...
    if (selectedImageIndex !== -1 && graduation.graduation_images?.length > selectedImageIndex) {
  ...
  const goToNextImage = () => {
    if (graduation?.graduation_images?.length) {
      setSelectedImageIndex((prev) => (prev + 1) % graduation.graduation_images.length);
    }
  };
  ...
  const goToPreviousImage = () => {
    if (graduation?.graduation_images?.length) {
      setSelectedImageIndex(
        (prev) => (prev - 1 + graduation.graduation_images.length) % graduation.graduation_images.length
      );
    }
  };
    ...
    <main className="mx-auto  mt-[180px] md:mt-24 sm:mt-10 max-w-7xl px-4 font-sans">
      ...
      >
        ...
        <div className="lg:w-1/2  flex flex-col gap-4  relative">
          <motion.div className="relative w-full aspect-[16/9] md:aspect-[4/3] overflow-hidden rounded-2xl cursor-pointer" layout>
            {currentImageUrl && (
              <motion.img
                src={currentImageUrl}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
              {graduation.graduation_images.map((img: any, index: number) => (
                <motion.div
                  ...
                  layout
                >
                  <img src={getImageUrl(img.image)} alt="Thumbnail" className="h-full w-full object-cover rounded-lg" />
                </motion.div>
              ))}
            </div>
          )}
        ...
        <div className="lg:absolute right-0 top-1 z-10 lg:w-1/2 flex flex-col items-start justify-center px-2 md:px-6 py-6 md:py-12 text-left">
          ...
          >
            ...
            />
            {graduation.graduation_year && <InfoCard label="Graduation Year" value={graduation.graduation_year} />}
          </motion.div>
        </div>
      </motion.div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-orange-100 mt-1 text-base md:text-lg">
                    {selectedStudent.father_name && `Son of ${selectedStudent.father_name}`}
                    {selectedStudent.grandfather_name && selectedStudent.father_name && `, Grandson of ${selectedStudent.grandfather_name}`}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 max-h-[calc(90vh-180px)] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start">
                      <span className="text-gray-500 font-medium min-w-[120px] text-sm md:text-base">Date of Birth:</span>
                      <span className="text-sm md:text-base">{new Date(selectedStudent.dob).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                    </div>
                    
                    <div className="flex items-start">
                      <span className="text-gray-500 font-medium min-w-[120px] text-sm md:text-base">Contact:</span>
                    ...
                    <div className="flex items-start">
                      <span className="text-gray-500 font-medium min-w-[120px] text-sm md:text-base">Graduation:</span>
                      <span className="bg-orange-100 text-orange-800 text-xs md:text-sm font-medium px-2.5 py-0.5 rounded">
                        {selectedStudent.graduation_type?.name || "Not specified"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start">
                    <span className="text-gray-500 font-medium min-w-[120px] text-sm md:text-base">Full Address:</span>
                    <span className="text-sm md:text-base">{selectedStudent.full_address || "Not provided"}</span>
                  </div>
                </div>
              </div>
