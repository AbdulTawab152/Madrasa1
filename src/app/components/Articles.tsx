  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {articles.map(article => (
        <Link key={article.id} href="/article" className="block p-6 bg-white rounded-lg shadow hover:shadow-lg transition">
          <h2 className="text-2xl font-semibold mb-2 text-gray-900">{article.title}</h2>
          <p className="text-gray-700 line-clamp-3">
            {article.excerpt || article.content || 'بدون توضیح'}
          </p>
        </Link>
      ))}
    </div>
...
export function ArticleSimple({ article }: { article: Article }) {
  return (
    <article className="bg-white p-8 rounded-lg shadow-lg max-w-3xl mx-auto font-sans">
      ...
      <section className="mb-8">
        ...
        <p className="text-gray-700 leading-relaxed">{article.content || 'بدون محتوا'}</p>
      </section>

      <footer className="border-t pt-4 text-gray-600">
        نوشته شده توسط {article.author?.name || 'ناشناس'}
      </footer>
    </article>
  );
}
