...
export default async function ArticleDetailsPage({ params }: Params) {
  const { slug } = params;

  // Fetch articles (still using ArticlesApi, you may rename it later)
    ...
    <main className="min-h-screen mt-32  bg-gradient-to-b from-amber-50/30 to-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-0 py-8">
        ...
        <nav className="flex mb-8 bg-white p-4 rounded-xl shadow-sm border border-amber-100">
          <ol className="flex items-center space-x-1 text-sm font-medium">
            ...
            <li className="flex items-center">
              ...
              <Link href="/articles" className="text-amber-600 hover:text-amber-700 transition-colors">Articles</Link>
            </li>
            {/* <li className="flex items-center">
              <svg className="w-4 h-4 mx-2 text-amber-400" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
              </svg>
              <span className="text-gray-700 font-medium truncate max-w-xs md:max-w-md">{article.title}</span>
            </li> */}
          </ol>
        </nav>
        ...
        <article className="mb-16">
          ...
          {/* Metadata */}
          {/* ... (same as before but replacing blog with article) */}
        </article>
